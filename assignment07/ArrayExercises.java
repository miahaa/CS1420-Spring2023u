...
public class ArrayExercises {
    ...
    public static void reverseOrder(char[] symbols) {
        for (int i = 0; i < symbols.length / 2; i++) {
            int oppositeI = symbols.length - i - 1;
            char temp = symbols[i];
            symbols[i] = symbols[oppositeI];
            symbols[oppositeI] = temp;
...
//        if(target == null || values == null)
        int count = 0;
        // count the ocurrence of the target object in the array
        for (int i = 0; i < values.length; i++) {
            ...
            else if (values[i] != null && values[i].equals(target))
                count++;
        }
        return count; //stub
    }
    ...
    public static double[] computeAreas(double[] widths, double[] heights) {
        if (widths == null || heights == null)
            throw new NullPointerException("Heights or widths is null");
        if (widths.length != heights.length)
            throw new ArrayIndexOutOfBoundsException("widths and heights are not the same size");

        // Create an array of rectangle areas
    ...
    public static Color[] remove(Color[] pixels, Color target) {
        ...
        for (int i = 0; i < pixels.length; i++) {
            if (target == null && pixels[i] != null)
                count ++;
            else if (pixels[i]!= null && target!= null && !pixels[i].equals(target))
                count++;
            else if (pixels[i] == null && target != null)
                count++;
        }
        Color[] removeArray = new Color[count];
        for (int i = 0, k = 0; i < pixels.length; i++) {
            if (target == null && pixels[i] != null)
            {
                removeArray[k++] = pixels[i];
            }
            else if (pixels[i] == null && target != null)
                removeArray[k++] = pixels[i];
            else if (target != null && pixels[i] != null && !pixels[i].equals(target)) {
                removeArray[k++] = pixels[i];
            }
        }
        return removeArray;
    ...
    public static Rectangle findSmallest(Rectangle[] rectangles) {
        ...
        for (int i = 1; i < rectangles.length; i++) {
            if (rectangles[i] == null) {
                throw new NullPointerException("rectangles must not contain null");
            }
            Rectangle current = rectangles[i];

            if (current.getWidth() * current.getHeight() < smallest.getWidth() * smallest.getHeight() ||
                    (current.getWidth() * current.getHeight() == smallest.getWidth() * smallest.getHeight() && i > 0 && i >= rectangles.length / 2)) {
                smallest = current;
                }
            }
    ...
    public static int[] histogram(int[] data) {
        ...
        // find the maximum value to find the length of the array
        int maxValue = 0;
        for (int value : data) {
            if (value >= maxValue) {
                maxValue = value;
        ...
        // Count how many times numbers appear in array
        for (int i = 0; i < data.length; i++) {
            int value = data[i];
            countArray[value]++;
        }
        return countArray;  //stub
    }
}
